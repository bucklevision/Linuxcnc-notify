...
mkdir -p "$BUILD/DEBIAN" "$BUILD/usr/bin" "$BUILD/usr/lib/python3/dist-packages" \
    "$BUILD/lib/systemd/system" "$BUILD/usr/share/linuxcnc-notify" \
    "$BUILD/usr/share/doc/linuxcnc-notify" "$OUT"
...
cp "$ROOT/VERSION" "$BUILD/usr/lib/python3/dist-packages/linuxcnc_notify/VERSION"
...
gzip -9 -c "$ROOT/CHANGELOG.md" > "$BUILD/usr/share/doc/linuxcnc-notify/changelog.gz"
